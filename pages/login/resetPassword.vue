      ...
      <div class="form-group relative pb-6">
        <ValidationProvider name="密碼" rules="required" v-slot="{ errors }" class="w-full">
          <div class="w-full relative">
            ...
            <fa class="absolute right-2 top-[11px] cursor-pointer text-base text-gray-600" :icon="['fas', `${pwdType === 'password' ? 'eye-slash' : 'eye'}`]" @click="showPassword('pwdId', 'pwdType')" />
          </div>
      ...
      <div class="form-group relative pb-6">
        <ValidationProvider name="再次輸入密碼" rules="required" v-slot="{ errors }" class="w-full">
          <div class="w-full relative">
            ...
            <fa class="absolute right-2 top-[11px] cursor-pointer text-base text-gray-600" :icon="['fas', `${againPwdType === 'password' ? 'eye-slash' : 'eye'}`]" @click="showPassword('againPwdId', 'againPwdType')" />
          </div>
          <span v-if="errors.length > 0" class="absolute left-0 bottom-1 text-red-500 text-xs">{{ errors[0] }}</span>
        </ValidationProvider>
      </div>
