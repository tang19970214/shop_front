        ...
        <li class="flex items-center py-1 text-gray-700 rounded text-lg" v-for="item in menuList" :key="item.id">
          <div class="w-auto flex flex-col">
            ...
            <div v-if="item.options" class="mt-1 pl-8">
              <ul>
                <li class="inline-block mb-1 tracking-wider text-base last:mb-0 cursor-pointer transition duration-300  hover:text-[#FA5936]" :class="{ 'text-[#FA5936]': items.path === $route.path }" v-for="items in item.options" :key="items.id" @click="goPath(items.path)">{{ items.label }}</li>
              </ul>
            </div>
          </div>
        </li>
