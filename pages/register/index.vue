<template>
  <section class="w-full">
    ...
    <ValidationObserver ref="form" v-if="defaultStep === 1">
      ...
      <button type="submit" data-mdb-ripple="true" data-mdb-ripple-color="light" class="w-full p-1.5 rounded-lg shadow-md text-white text-lg tracking-widest bg-gradient-to-r from-[#FA5936] to-[#FF6D3F] hover:shadow-inner mb-4 disabled:cursor-not-allowed disabled:opacity-80" :disabled="disNextStep" @click="nextStep()">
        <fa v-if="disNextStep" class="animate-spin text-xl mr-2" :icon="['fas', 'spinner']" />下一步
      </button>
      ...
      <p class="text-gray-800 mt-6 text-center">
        已是會員？
        <a class="text-[#0EA5E9] hover:underline cursor-pointer" @click="$router.push({ name: 'login' })">前往登入</a>
      </p>
    </ValidationObserver>
    ...
    <ValidationObserver ref="vertify" v-if="defaultStep === 2">
      ...
      <button type="submit" data-mdb-ripple="true" data-mdb-ripple-color="light" class="w-full p-1.5 rounded-lg shadow-md text-white text-lg tracking-widest bg-gradient-to-r from-[#FA5936] to-[#FF6D3F] hover:shadow-inner disabled:cursor-not-allowed disabled:opacity-80" :disabled="disGoVerify" @click="goVerify()">
        <fa v-if="disGoVerify" class="animate-spin text-xl mr-2" :icon="['fas', 'spinner']" />驗證
      </button>
    </ValidationObserver>
  </section>
</template>
