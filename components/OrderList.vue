<template>
  <div>
    <div class="w-full block lg:flex text-[18px] mt-[30px] py-[11px] px-[17px]" :class="{ 'bg-[#FFF0E5]': order.orderType !== '已取消', 'bg-[#EAEAEA]': order.orderType === '已取消' }">
      ...
      <div class="flex ml-auto">
        <span> 狀態： </span>
        <span :class="{ 'text-[#13823b]': order.orderType === '訂單已確認', 'text-[#f0ad4e]': order.orderType === '訂單處理中', 'text-[#fff]': order.orderType === '訂單已完成', 'text-[#000]': order.orderType === '已取消' }">
          {{ order.orderType }}
        </span>
      </div>
    </div>
    <table class="w-full relative">
      <tr v-for="(list, idx) in orderList" :key="list.id" class="border-b-[1px] border-b-[#bbbbbb] duration-150 hover:bg-stone-50 cursor-pointer">
        <td class="py-[15px]" :class="{ hidden: idx >= 5 && isOverFiveItems && !isShowTotalItems }">
          <div class="flex">
            ...
            <div class="flex flex-col justify-center">
              ...
              <p class="block lg:hidden text-[18px] text-[#FA5936] mt-[5px]">${{ list.sale * list.quantity }}</p>
            </div>
          </div>
        </td>
        <td class="hidden lg:table-cell" :class="{ hidden: idx >= 5 && isOverFiveItems && !isShowTotalItems }">
    ...
    <div class="w-full text-right mt-[15px] lg:flex lg:items-center lg:justify-end text-[18px]" :class="{ 'lg:hidden': isOverFiveItems && !isShowTotalItems, hidden: isOverFiveItems && !isShowTotalItems }">
      ...
      <div class="flex justify-end space-x-[15px] mt-[13px] lg:mt-[0px]">
        ...
        <span class="font-bold">
          訂單金額：
          <span class="text-[#FA5936] text-[18px]"> ${{ order.totalPrice }} </span>
        </span>
      </div>
    </div>
  </div>
